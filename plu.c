/** Plu.c: LU desompostion with Partial Pivoting Using Threads**/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <pthread.h>
...
#define N 8
int n;
pthread_barrier_t barrier; //declaring barrier
...
void *lu(void *arg)//changed to function pointer to be compatible with threads
{
    int i, j, k, m, itemp;
    double max;
    int myid = (int)arg;//thread #s
    double temp;
    double si;
    ...
    for (k = 0; k < n; k++)
    {
        ...
        pthread_barrier_wait(&barrier);//waiting for threads
    }
}
...
//matricies initialized and displayed
    ...
    printV('X', X);//printing final solution
}